    ...
    try {
      ...
      async function scrapeCurrentPage () {
        ...
        const imgs = imgsRaw.map((img) => {
          ...
          const imgURL = `${imgBaseURL}?${photoIdParam[0]}`
      ...
      for (let i = 0; i < imgsArr.length; i++) {
        result = await download(imgsArr[i], `image-${i}.jpeg`)

        if (result === true) {
          console.log('Success:', imgsArr[i], 'has been downloaded successfully.')
        } else {
          errors.push(imgsArr[i])
          console.log('Error:', imgsArr[i], 'was not downloaded.')
